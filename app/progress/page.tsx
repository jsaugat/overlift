...
export default function ProgressPage() {
  ...
  const logWeight = async () => {
    ...
    await supabase
      .from('weight_logs')
      .upsert({ log_date: today, weight_kg: val }, { onConflict: 'log_date' })
    setInput('')
    await fetchLogs()
    setSaving(false)
  }
      ...
      <div className="bg-app border border-[rgb(var(--border))] rounded-xl mt-2 overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          <div>
            ...
            <p className="text-sm text-muted mt-0.5">Log your bodyweight weekly to track your lean bulk.</p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <MetricCard
              label="Change"
              value={change !== null ? `${parseFloat(change) >= 0 ? '+' : ''}${change} kg` : '—'}
            />
          ...
          <div className="rounded-xl p-4 border-l-[3px] border-[rgb(var(--blue))] bg-[rgb(var(--blue-bg))]">
            ...
            <p className="text-xs text-muted leading-relaxed">
              Aim for <strong className="text-app">+0.25–0.5 kg/month</strong> weight gain. Gaining faster → reduce calories by ~100. Too slow after 3 weeks → add ~100 cals.
            </p>
          </div>
        </div>
      </div>
