...
const PROTEIN_FOODS = [
  'Eggs (6–8/day)', 'Chicken / fish', 'Dal (lentils)', 'Paneer / chhurpi',
  'Dahi / Greek yogurt', 'Soya chunks', 'Milk / whey', 'Chickpeas / rajma',
]
...
export default function NutritionPage() {
  ...
  const [logInput, setLogInput] = useState({ calories: '', protein: '', carbs: '', fat: '' })
    ...
    <>
      ...
      <div className="bg-app border border-app rounded-xl mt-2 overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          <div>
            <h2 className="text-base font-medium text-app">Nutrition targets</h2>
          ...
          <div className="grid grid-cols-2 gap-2">
            <MetricCard label="Daily calories"   value={`${macros.calories} kcal`} />
            <MetricCard label="Protein target"   value={`${macros.protein} g`} />
            <MetricCard label="Maintenance est." value={`${macros.maintenance} kcal`} />
          ...
          <div className="bg-app border border-app rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-muted block mb-1.5">Activity level</label>
                <select
                  value={activity}
              ...
              <div>
                <label className="text-xs text-muted block mb-1.5">Surplus</label>
                <select
                  value={surplus}
          ...
          <div className="bg-app border border-app rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              {(['calories', 'protein', 'carbs', 'fat'] as const).map((field) => (
                <div key={field}>
                  <label className="text-xs text-muted block mb-1 capitalize">{field}</label>
                  <input
                    type="number"
          ...
          <div className="rounded-xl p-4 border-l-[3px] border-pull badge-pull">
            ...
            <p className="text-xs text-muted leading-relaxed">
              Target <strong className="text-app">7–8 hours</strong> every night. Muscle grows during sleep — prioritise it as much as training.
            </p>
          </div>
        </div>
      </div>
