...
export function ExerciseList({ day }: ExerciseListProps) {
   ...
   */
  const saveToDb = useCallback(async (ex: ProgramExercise, idx: number) => {
  ...
  const done = day.exercises.filter((_, i) => checked[`${day.key}-${i}`]).length
  ...
  return (
    <div className="bg-app border border-[rgb(var(--border))] rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[rgb(var(--border))]">
        <span className={cn('text-[11px] font-medium px-2.5 py-1 rounded-lg', day.badgeClass)}>
          {day.type} day
        </span>
        ...
        {day.exercises.map((ex, i) => {
          ...
          return (
            <div key={id} className="flex gap-3 px-4 py-3 items-start">
              ...
              >
                {isDone && <Check size={11} className="text-[rgb(var(--green))]" strokeWidth={3} />}
              </button>

              {/* Info */}
              <div className="flex-1 min-w-0">
                <div className={cn('text-sm font-medium text-app', isDone && 'line-through text-faint')}>
                  {ex.name}
                </div>
